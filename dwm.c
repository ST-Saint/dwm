...
setlayout(const Arg *arg)
{
	if (!arg || !arg->v || arg->v != selmon->lt[selmon->sellt])
		selmon->sellt = selmon->pertag->sellts[selmon->pertag->curtag] = (selmon->sellt + 1) % LENGTH(layouts);
	if (arg && arg->v)
...
toggleview(const Arg *arg)
{
	...
	if (newtagset) {
		...
		selmon->lt[2] = selmon->pertag->ltidxs[selmon->pertag->curtag][2];
...
updatesystray(void)
{
	...
	XChangeProperty(
	    dpy, systray->win, netatom[NetSystemTrayOrientation], XA_CARDINAL,
	    32, PropModeReplace,
